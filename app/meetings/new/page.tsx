...
function DateRangePicker({
  startDate, endDate, onRangeChange, minDate,
}: {
  ...
  const handleClick = (ymd: string) => {
    ...
    if (phase === "start") {
      onRangeChange(ymd, ""); setPhase("end");
    } else {
      if (!startDate || ymd < startDate) { onRangeChange(ymd, ""); setPhase("end"); }
      else { onRangeChange(startDate, ymd); setPhase("start"); }
    }
  };
    ...
    <div className="neu-card overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4"
        ...
        <button type="button" onClick={prevMonth}
          className="w-9 h-9 rounded-xl neu-raised-sm flex items-center justify-center hover:-translate-y-0.5 transition-transform duration-200">
      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-7 mb-2">
          {WEEKDAYS.map((w) => (
            <div key={w} className="text-center text-xs font-semibold text-muted py-1">{w}</div>
          ))}
        </div>
          ...
          {cells.map((day, idx) => {
            ...
            if (isStart || isEnd) {
              ...
              spanStyle = {
                background: "#44DCEA",
                color: "#0C0C0F",
                boxShadow: "0 0 12px rgba(68,220,234,0.4)",
              };
            ...
            return (
              ...
              >
                <span
                  className={`w-9 h-9 flex items-center justify-center rounded-xl text-sm select-none transition-all duration-150 ${
                    isPast ? "text-muted/25" : ""
                  } ${dayClass}`}
                  style={spanStyle}
                >
                  {day}
      ...
      <div className="px-4 pb-4">
        <div className="neu-inset-sm rounded-xl px-5 py-3 text-sm flex items-center justify-between min-h-[44px]">
          {!startDate ? (
    ...
    <div className="min-h-screen bg-base">
      ...
      <main className="max-w-2xl mx-auto px-6 py-10">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="font-display text-3xl font-bold text-fore tracking-tight">새 약속 잡기</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" name="name" value={formData.name} onChange={handleChange}
                  placeholder="홍길동" className="neu-input" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-fore mb-2">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" name="title" value={formData.title} onChange={handleChange}
                  placeholder="예: 팀 회식, 스터디, 동창 모임" className="neu-input" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-fore mb-2">
                ...
                <textarea name="description" value={formData.description} onChange={handleChange}
                  ...
                  rows={3} className="neu-input resize-none" />
              </div>
            </div>
          </div>
          ...
          <div className="neu-card p-7">
            ...
            <DateRangePicker
              ...
              onRangeChange={handleRangeChange} minDate={today}
            />
          </div>
          ...
          <div className="neu-card p-7">
            ...
            <input type="date" name="deadline" value={formData.deadline} onChange={handleChange}
              min={today} className="neu-input" />
            <p className="text-xs text-muted mt-3 ml-1">이 날짜까지 참가자들이 응답할 수 있습니다.</p>
          </div>

          <button type="submit" disabled={loading} className="neu-btn neu-btn-primary w-full py-4 text-base">
            {loading ? "생성 중..." : "약속 잡기 →"}
          </button>
        </form>
      </main>
